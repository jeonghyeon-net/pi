...
export function buildRpcBody(toolName: string, args: Record<string, unknown>): string {
...
export function parseSseResponse(body: string): McpRpcResponse | null {
	for (const line of body.split("\n")) {
		if (!line.startsWith("data:")) continue;
		const payload = line.slice(5).trim();
		if (!payload) continue;
		try {
			const c = JSON.parse(payload) as McpRpcResponse;
			if (c?.result || c?.error) return c;
		} catch {}
	}
	try {
		const c = JSON.parse(body) as McpRpcResponse;
		if (c?.result || c?.error) return c;
	} catch {}
	return null;
}

export function extractText(parsed: McpRpcResponse): string {
	if (parsed.error) {
