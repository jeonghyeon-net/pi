...
export async function fetchContent(
	...
	if (!res.ok) return { url, title: "", content: "", error: `HTTP ${res.status}: ${res.statusText}` };
	const ct = res.headers.get("content-type") || "";
	if (isBinary(ct)) return { url, title: "", content: "", error: `Unsupported: ${ct.split(";")[0]}` };
	...
	if (!isHtml(ct)) return { url, title: url, content: text, error: null };
	const result = htmlToMarkdown(text);
	if (!result) return { url, title: "", content: "", error: "Could not extract readable content" };
