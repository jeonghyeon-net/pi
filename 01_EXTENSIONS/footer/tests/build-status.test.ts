...
describe("buildFooterStatusEntries", () => {
	...
	it("prepends session name", () => { const ctx = mockCtx({ sessionManager: { getCwd: () => "/t", getSessionName: () => "s" } }); expect(buildFooterStatusEntries(ctx, mockFooterData())).toEqual([[NAME_STATUS_KEY, "s"]]); });
